...
void movementPhase(struct Game *ptr, struct Tile board[][MAX_SIZE], struct Player players[])
{
    ...
    // as long as there are free penguins on the board
    do
    {
        if (players[ptr->currentPlayer].freePenguins == 0) // change current player to the one that can still move penguins (if other players placed their penguins on floes from which they cannot move)
            do
            {
                swapPlayer(ptr);
            } while (players[ptr->currentPlayer].freePenguins == 0);

        selected = selectPenguin(ptr, board, players);
        movePenguin(ptr, board, selected, players);

        if (ptr->freePenguins != 0)
            do
            {
                swapPlayer(ptr);
            } while (players[ptr->currentPlayer].freePenguins == 0); // swap to the player that still has free penguins
